...
def _init_swanlab(cfg):
    if not comm.is_main_process() or not _env_flag("VLPART_SWANLAB_ENABLED", False):
        return None
    if not SWANLAB_AVAILABLE or swanlab is None:
        logger.warning("SwanLab logging requested but swanlab is not available: %s", SWANLAB_IMPORT_ERROR)
        return None

    project = os.getenv("VLPART_SWANLAB_PROJECT", "VLPart")
    exp_name = os.getenv(
        "VLPART_SWANLAB_EXP_NAME",
        os.path.basename(os.path.normpath(cfg.OUTPUT_DIR)) or "vlpart_train",
    )
    config = {
        ...
        "dataset_bs": list(cfg.DATALOADER.DATASET_BS),
        "fp16": cfg.FP16,
        "frozen_swin_bottom_up": _env_flag("VLPART_FREEZE_SWIN_BOTTOM_UP", False),
    }

    try:
        swanlab.init(project=project, experiment_name=exp_name, config=config)
        logger.info("SwanLab logging enabled: project=%s experiment=%s", project, exp_name)
        return swanlab
    except Exception as e:
        logger.warning("SwanLab init failed (%s), training will continue without SwanLab logging", e)
        return None


def _log_swanlab(swanlab_logger, metrics, step):
    if swanlab_logger is None:
        return
    try:
        swanlab_logger.log(metrics, step=step)
    except Exception as e:
        logger.warning("SwanLab log failed at step %s: %s", step, e)
...
def do_train(cfg, model, resume=False):
    model.train()
    swanlab_logger = _init_swanlab(cfg)
    swanlab_log_period = max(1, int(os.getenv("VLPART_SWANLAB_LOG_PERIOD", "20")))
    ...
    with EventStorage(start_iter) as storage:
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            if comm.is_main_process() and (
                ...
                or (iteration + 1) % swanlab_log_period == 0
                ...
                swanlab_metrics = {
                    ...
                    "metrics/data_secs_per_iter": data_time,
                    "metrics/total_secs_per_iter": iter_time,
                }
                for loss_name, loss_value in loss_dict_reduced.items():
                    swanlab_metrics["train/{}".format(loss_name)] = loss_value
                if torch.cuda.is_available():
                    swanlab_metrics.update({
                        "memory/max_allocated_mb": torch.cuda.max_memory_allocated() / (1024 ** 2),
                        "memory/max_reserved_mb": torch.cuda.max_memory_reserved() / (1024 ** 2),
                    })
                _log_swanlab(swanlab_logger, swanlab_metrics, iteration + 1)
    ...
    if comm.is_main_process() and swanlab_logger is not None and hasattr(swanlab_logger, "finish"):
        try:
    ...
    if comm.is_main_process() and torch.cuda.is_available():
        logger.info(
            "Peak CUDA memory during training: allocated=%.1f MiB reserved=%.1f MiB",
