...
from vigor_vlpart_topk_reranker import (  # noqa: E402
    containment_score,
    parse_llmseg_feature_from_filename,
)


FEATURE_NAMES = [
    "llmseg_similarity",
    "llmseg_predicted_iou",
    "vlpart_score",
    "containment_score",
]
FEATURE_WEIGHTS = np.asarray([0.25, 0.25, 0.25, 0.25], dtype=np.float32)



def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        "Equal-weight bounded score selection on LLMSeg top-K masked RGB candidates"
    )
    ...
    parser.add_argument("--output_dir", default="vlpart_vigor_outputs/llmseg_topk_weighted_score/results")
    parser.add_argument("--vis_dir", default="vlpart_vigor_outputs/llmseg_topk_weighted_score/visualizations")
    parser.add_argument("--pred_masks_dir", default="vlpart_vigor_outputs/llmseg_topk_weighted_score/pred_masks")
...
def scale_feature_matrix(features: np.ndarray) -> np.ndarray:
    features = np.asarray(features, dtype=np.float32)
    scaled = np.zeros_like(features, dtype=np.float32)
    if features.size == 0:
        return scaled
    scaled[:, 0] = (features[:, 0] + 1.0) * 0.5
    scaled[:, 1:] = features[:, 1:]
    return np.clip(scaled, 0.0, 1.0)

def evaluate_sample_by_weighted_score(
    sample: Dict,
    ...
    if not topk_mask_paths:
        raise FileNotFoundError(
            "No LLMSeg top-K masks found for {} {} obj{} instr{}".format(
                sample["img_name"],
                sample.get("gt_object", ""),
                sample.get("obj_count", 1),
                sample.get("instruction_index", 0),
            )
        )
    ...
    for rank, topk_mask_path in topk_mask_paths:
        ...
        cache_key = f"{image_path}|{topk_mask_path}|all_weighted_score_candidates"
        ...
        if cached is None:
            ...
            pred_cache.put(cache_key, (candidates, masked_bgr, object_fg))
        else:
            candidates, masked_bgr, object_fg = cached
        ...
        for candidate in candidates:
            contain = containment_score(candidate["pred_fg"], object_fg)
            features = [
                float(pred_similarity),
                float(pred_iou),
            ...
            candidate_rows.append(
                {
                    ...
                    "features": features,
                    "pred_similarity": float(pred_similarity),
                    "pred_iou": float(pred_iou),
                    "pred_iou_available": pred_iou_available,
                    "containment_score": float(contain),
                }
            )

    if not candidate_rows:
        raise RuntimeError("No valid weighted-score candidates")

    feature_matrix = np.asarray([row["features"] for row in candidate_rows], dtype=np.float32)
    scaled_features = scale_feature_matrix(feature_matrix)
    weighted_scores = np.clip(scaled_features @ FEATURE_WEIGHTS, 0.0, 1.0)
    best_idx = int(np.argmax(weighted_scores))
    best = candidate_rows[best_idx]
    best["scaled_features"] = scaled_features[best_idx].astype(float).tolist()
    best["weighted_score"] = float(weighted_scores[best_idx])
    best["candidate_count"] = len(candidate_rows)
...
def evaluate_split(
    ...
    for ordinal, sample in enumerate(tqdm(samples, desc=split_name.capitalize()), start=1):
        ...
        try:
            best = evaluate_sample_by_weighted_score(sample, demo, args, pred_cache)
            iou = best["iou"]
            ...
            results.append(
                {
                    ...
                    "scaled_features": best["scaled_features"],
                    "llmseg_similarity": best["pred_similarity"],
                    "llmseg_predicted_iou": best["pred_iou"],
                    "pred_iou_available": best["pred_iou_available"],
...
def write_sample_result(f, split_title: str, index: int, result: Dict) -> None:
    raw_features = ", ".join(
        f"{name}={value:.4f}" for name, value in zip(FEATURE_NAMES, result["features"])
    )
    scaled_features = ", ".join(
        f"{name}={value:.4f}"
        for name, value in zip(FEATURE_NAMES, result["scaled_features"])
    )
    f.write("[{} Instruction Sample {}]\n".format(split_title, index))
    f.write("  Image: {}\n".format(result["image_path"]))
    f.write("  GT Affordance Mask: {}\n".format(result["gt_mask_path"]))
    f.write("  Object: {}\n".format(result["gt_object"]))
    f.write("  Instruction {}: {}\n".format(result["instruction_index"], result.get("instruction", "")))
    f.write("  IC-IoU: {:.4f}\n".format(result["avg_ic_iou"]))
    f.write("  SSR Success: {}\n".format(int(result["success"])))
    f.write("  Weighted Score: {:.4f}\n".format(result["weighted_score"]))
    f.write("  Raw Features: {}\n".format(raw_features))
    f.write("  Scaled Features: {}\n".format(scaled_features))
    f.write("  Pred IoU Available: {}\n".format(int(result["pred_iou_available"])))
    f.write("  Pred Class: {} ({})\n".format(result["pred_class_name"], result["pred_class"]))
    f.write("  Selected TopK Rank: {}\n".format(result["llmseg_topk_rank"]))
    f.write("  VLPart Instance Index: {}\n".format(result["vlpart_instance_index"]))
    f.write("  Candidate Count: {}\n".format(result["candidate_count"]))
    f.write("  LLMSeg TopK Mask: {}\n".format(result["llmseg_topk_mask_path"]))
    if result.get("pred_mask_path"):
        f.write("  Pred Mask: {}\n".format(result["pred_mask_path"]))
    if result.get("vis_path"):
        f.write("  Visualization: {}\n".format(result["vis_path"]))
    f.write("\n")
...
def write_results(args, easy_results, hard_results, skipped_errors, easy_icr_records, hard_icr_records):
    ...
    result_file = str(
        Path(args.output_dir) / f"vlpart_llmseg_topk_weighted_score_results_{time.strftime('%Y%m%d_%H%M%S')}.txt"
    )
    with open(result_file, "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("  VLPart VIGOR-100K LLMSeg Top-K Weighted-Score Selection Results\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write("Selection: highest equal-weight score over bounded features\n")
        f.write("Features: llmseg_similarity, llmseg_predicted_iou, vlpart_score, containment_score.\n")
        f.write("llmseg_similarity is mapped from [-1, 1] to [0, 1]; the other three features are used directly and clipped to [0, 1].\n")
        ...
        f.write(
            f"| WeightedScore | {easy_metrics['ic_iou']:.4f} | "
