"""
LLMSeg (LISA-based) VIGOR-100K Unseen 数据集测试脚本

使用 finetune_llmseg_vigor_simple.py 训练的模型进行 unseen 数据集测试
计算 IC-IoU、ICR 和 SSR@0.5 指标
...
使用方法:
    python test_llmseg_vigor_unseen.py --checkpoint /path/to/ckpt_model --sam_masks_dir /path/to/sam_masks
"""
...
def parse_args(args):
    parser = argparse.ArgumentParser(description="LLMSeg VIGOR-100K Unseen 测试脚本")
    ...
    # 数据集路径
    parser.add_argument("--data_dir", default="/opt/data/private/LLMSeg/dataset/VIGOR-100K/unseen",
                        type=str, help="Unseen 数据集路径")
    parser.add_argument("--sam_masks_dir", required=True, type=str,
                        help="SAM 候选 mask 目录 (必需)")
    
    # 输出路径
    parser.add_argument("--output_dir", default="./result_unseen",
    ...
    # SSR 阈值配置
    parser.add_argument("--ssr_threshold", default=0.5, type=float,
                        help="SSR 成功率阈值")
    
    # 可视化配置
    parser.add_argument("--vis_dir", default="./vis_output_unseen", type=str,
                        help="可视化输出目录")
...
def save_visualization(
    ...
    # 绘制文字
    # 第一行：GT Object 名称
    title_text = f"GT Object: {gt_object} | Difficulty: {difficulty} | Unseen"
...
def predict_mask_llmseg(
    ...
    """使用 LLMSeg 模型预测 mask（从 SAM 候选中选择）"""
    img_name = getattr(args, '_current_img_name', None)
    ...
    try:
        ...
        if segs is None or segs_origin is None:
            if args.debug:
                print(f"  [警告] SAM mask 为空")
            return None
    except Exception as e:
        if args.debug:
            print(f"  [警告] 获取 SAM mask 失败: {e}")
        return None
...
def evaluate_sample(
    ...
    pred_masks = []
    ic_ious = []
    all_instruction_ious = []  # 用于 SSR 计算
    vis_saved = 0  # 可视化保存成功数
    vis_failed = 0  # 可视化保存失败数
    ...
    for instr_idx, instruction in enumerate(instructions[:3]):
        ...
        # 调整大小
        if pred_mask is not None and pred_mask.shape != gt_mask.shape:
        ...
        if pred_mask is not None:
            ...
            ic_ious.append(iou)
            all_instruction_ious.append(iou)
            if args.debug:
                print(f"    [调试] 指令{instr_idx}: pred_mask 获取成功, IoU={iou:.4f}")
            ...
            if args.save_vis:
                ...
                try:
                    ...
                    vis_saved += 1
                    if args.debug:
                        print(f"    [调试] 可视化已保存: {output_path}")
                except Exception as vis_e:
                    vis_failed += 1
                    if args.debug:
                        print(f"    [警告] 可视化保存失败: {vis_e}")
        else:
            if args.debug:
                print(f"    [调试] 指令{instr_idx}: pred_mask 为 None")
    ...
    return {
        ...
        'iou_pairs': iou_pairs,
        'all_instruction_ious': all_instruction_ious,  # 用于 SSR 计算
        'num_instructions': len(instructions[:3]),
        'vis_saved': vis_saved,  # 可视化保存成功数
        'vis_failed': vis_failed,  # 可视化保存失败数
    }
...
def main(args):
    ...
    # 加载测试数据（unseen 只有 easy）
    print("\n" + "=" * 60)
    print("  加载 Unseen 测试数据...")
    print("=" * 60)
    ...
    print(f"  save_vis 参数: {args.save_vis}")
    print(f"  vis_dir 参数: {args.vis_dir}")

    
    # 存储结果
    results = []
    all_instruction_ious = []  # 收集所有指令的 IoU 用于 SSR 计算
    ...
    # 测试 Easy 样本
    print("\n" + "=" * 60)
    print("  测试 Unseen Easy 样本")
    print("=" * 60)
    
    for sample in tqdm(easy_samples, desc="Unseen Easy"):
        try:
            ...
            if result:
                ...
                all_instruction_ious.extend(result['all_instruction_ious'])
                vis_saved_count += result.get('vis_saved', 0)
                vis_failed_count += result.get('vis_failed', 0)
        except Exception as e:
            if args.debug:
    ...
    if args.save_vis:
        print(f"\n  可视化保存统计:")
        print(f"    成功: {vis_saved_count}")
        print(f"    失败: {vis_failed_count}")
    ...
    metrics = compute_metrics(results, icr_thresholds)
    
    # 计算 SSR@0.5 (Success Rate @ IoU threshold 0.5)
    ssr_threshold = args.ssr_threshold
    total_instructions = len(all_instruction_ious)
    successful_instructions = sum(1 for iou in all_instruction_ious if iou >= ssr_threshold)
    ssr = successful_instructions / total_instructions if total_instructions > 0 else 0.0
    
    # 保存结果
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    result_file = os.path.join(args.output_dir, f"llmseg_vigor_unseen_results_{timestamp}.txt")
    
    with open(result_file, "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("  LLMSeg VIGOR-100K Unseen 测试结果\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write(f"IC-IoU (Unseen Easy): {metrics['ic_iou']:.4f}\n\n")
        
        f.write(f"SSR@{ssr_threshold} (Success Rate @ IoU >= {ssr_threshold}):\n")
        f.write(f"  成功指令数: {successful_instructions} / {total_instructions}\n")
        f.write(f"  成功率: {ssr:.4f} ({ssr*100:.2f}%)\n\n")
        
        f.write("ICR@0.X:\n")
        for t in icr_thresholds:
            f.write(f"  ICR@{t:.1f}: {metrics['icr_scores'][t]:.4f}\n")
        ...
        header = "| Dataset  | Methods | IC-IoU | SSR@0.5 |"
        for t in icr_thresholds:
            header += f" ICR@{t:.1f} |"
        f.write(header + "\n")
        
        row = f"| Unseen   | LLMSeg  | {metrics['ic_iou']:.4f} | {ssr:.4f}  |"
        for t in icr_thresholds:
            row += f" {metrics['icr_scores'][t]:.4f} |"
        f.write(row + "\n\n")
    ...
    # 打印汇总
    print("\n" + "=" * 60)
    print("  测试完成 - Unseen 结果汇总")
    print("=" * 60)
    print(f"总样本数: {metrics['count']}")
    print(f"总指令数: {total_instructions}")
    print(f"IC-IoU (Unseen Easy): {metrics['ic_iou']:.4f}")
    print(f"SSR@{ssr_threshold}: {ssr:.4f} ({ssr*100:.2f}%)")
    print(f"ICR@0.5: {metrics['icr_scores'].get(0.5, 0.0):.4f}")
