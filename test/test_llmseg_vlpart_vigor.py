#!/usr/bin/env python
"""Two-stage VIGOR evaluation with Top-K VLPart candidate reranking.
...
def load_reranker_module():
    sys.path.insert(0, str(VLPART_ROOT / "tools"))
    sys.path.insert(0, str(VLPART_ROOT))
    sys.path.insert(0, str(VLPART_ROOT / "demo"))
    return load_module(
        "vlpart_topk_reranker_base",
        VLPART_ROOT / "tools" / "vigor_vlpart_topk_reranker.py",
    )
...
def infer_image_path(data_dir: str, sample: Dict, first_gt_mask_path: str) -> Tuple[str, str]:
    image_path_raw = sample.get("image_path", sample.get("image", ""))
    if image_path_raw:
        image_path = resolve_data_path(data_dir, image_path_raw)
        return image_path, os.path.basename(image_path)

    scene = str(sample.get("scene", "")).strip()
    if scene:
        image_name = os.path.basename(scene)
        if Path(image_name).suffix == "":
            image_name = f"{image_name}.png"
        return str(Path(data_dir) / image_name), image_name

    first_mask_filename = os.path.basename(first_gt_mask_path)
    match = re.match(r"^(\d+)_", first_mask_filename)
    if not match:
        raise ValueError(f"Cannot infer image name from GT mask: {first_gt_mask_path}")
    image_name = f"{match.group(1)}.png"
    return str(Path(data_dir) / image_name), image_name


def load_eval_samples_from_gt_mask_path(
    data_dir: str,
    json_file_name: str,
    difficulty: str,
) -> List[Dict]:
    """Load VIGOR samples for VLPart evaluation using gt_mask_path as GT."""
    ...
    for sample in raw_samples:
        gt_mask_paths_rel = split_path_list(sample.get("gt_mask_path", ""))
        if not gt_mask_paths_rel:
            continue

        image_path, img_name = infer_image_path(data_dir, sample, gt_mask_paths_rel[0])
        if not os.path.exists(image_path):
            continue

        gt_mask_paths = [resolve_data_path(data_dir, path) for path in gt_mask_paths_rel]
        gt_object_mask_paths = [
            resolve_data_path(data_dir, path)
            for path in split_path_list(sample.get("gt_object_mask_path", ""))
        ]
...
def topk_metadata_path(args: argparse.Namespace, split_name: str, stem: str) -> Path:
    return Path(args.llmseg_masks_dir) / split_name / f"{stem}_topk.json"


def topk_mask_path(args: argparse.Namespace, split_name: str, stem: str, rank: int) -> Path:
    return Path(args.llmseg_masks_dir) / split_name / f"{stem}_top{rank}.png"


def topk_region_path(args: argparse.Namespace, split_name: str, stem: str, rank: int) -> Path:
    return Path(args.region_rgb_dir) / split_name / f"{stem}_top{rank}.png"


def save_topk_metadata(path: Path, payload: Dict) -> None:
...
def save_gt_pred_mask_comparison(
    ...
    gt_mask_name = os.path.basename(sample_info.get("gt_mask_path", ""))
    text_height = 120 if gt_mask_name else 100
    ...
    metric_y = 75
    if gt_mask_name:
        gt_display = gt_mask_name[:80] + "..." if len(gt_mask_name) > 80 else gt_mask_name
        cv2.putText(combined_with_text, f"GT mask: {gt_display}", (10, 75), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
        metric_y = 100
    cv2.putText(combined_with_text, f"IoU: {iou:.4f}", (10, metric_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
    cv2.putText(combined_with_text, "GT gt_mask_path (Green)", (200, metric_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 128, 0), 2)
    cv2.putText(combined_with_text, "VLPart overlay (Red)", (430, metric_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
...
def write_results(
    ...
    with open(result_file, "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("  LLMSeg Top-K + VLPart + Reranker VIGOR-100K 测试结果\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write(f"LLMSeg Top-K K: {args.topk_mask_k}\n")
        f.write(f"VLPart candidate mode: {args.vlpart_mask_selection}\n")
        f.write(f"Reranker checkpoint: {args.reranker_checkpoint}\n")
        f.write("GT mask source: gt_mask_path\n")
        f.write("VLPart input mode: one region_rgb per LLMSeg Top-K rank\n")
        f.write(
            "Reranker features: pred_similarity, pred_iou, vlpart_score, "
            "containment_score\n"
        )
        ...
        row = (
            f"| Offline  | LLMSegTopK+VLPart+Reranker | {easy_metrics['ic_iou']:.4f}      | "
        ...
        for split_name, split_title in [("easy", "Easy"), ("hard", "Hard")]:
            ...
            for index, result in enumerate(results[split_name], start=1):
                ...
                f.write(f"  GT Source: {result.get('gt_mask_source', 'gt_mask_path')}\n")
                ...
                for detail in result.get("selection_details", []):
                    f.write(
                        f"  Instruction {detail['instruction_idx']} selection: "
                        f"topK_rank={detail['selected_topk_rank']}, "
                        f"vlpart_instance={detail['vlpart_instance_index']}, "
                        f"candidates={detail['candidate_count']}, "
                        f"reranker_score={detail['reranker_score']:.4f}\n"
                    )
                    f.write(
                        "    Features [pred_similarity, pred_iou, vlpart_score, "
                        f"containment_score]: {[f'{x:.4f}' for x in detail['features']]}\n"
                    )
                    f.write(f"    LLMSeg mask: {detail['llmseg_mask_path']}\n")
                for threshold in icr_thresholds:
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--topk_mask_k", default=3, type=int)
    ...
    parser.add_argument(
        "--vlpart_mask_selection",
        default="all",
        choices=["top1", "all", "union"],
        help="Candidate mode per LLMSeg rank; use all for the trained candidate reranker.",
    )
    ...
    parser.add_argument("--reranker_device", default="cuda:0")
    parser.add_argument(
        "--vlpart_vocabulary",
        default="custom",
        choices=[
    ...
    # predict_mask_llmseg also constructs a legacy single-mask output. This
    # pipeline ignores it and consumes only similarity-ranked top_masks, but
    # the shared helper still expects these internal compatibility fields.
    args.mask_selection_strategy = "similarity_top1"
    args.pred_iou_threshold = 0.5
    args.selection_topk = 1
    return args


def load_split_samples(_llmseg, args: argparse.Namespace) -> Dict[str, List[Dict]]:
    samples = {"easy": [], "hard": []}
    if args.split in ["easy", "both"]:
        samples["easy"] = load_eval_samples_from_gt_mask_path(
            args.data_dir, args.easy_json_file, "easy"
        )
    if args.split in ["hard", "both"]:
        samples["hard"] = load_eval_samples_from_gt_mask_path(
            args.data_dir, args.hard_json_file, "hard"
        )
...
def maybe_load_cached_region_inputs(
        ...
        for sample in samples[split_name]:
            try:
                ...
                for instruction_idx, instruction in enumerate(sample.get("instructions", [])[:3]):
                    ...
                    metadata_path = topk_metadata_path(args, split_name, stem)
                    ...
                    try:
                        ...
                        continue
                    if (
                        int(metadata.get("version", 0)) != 1
                        or int(metadata.get("requested_topk_mask_k", 0)) != int(args.topk_mask_k)
                        or metadata.get("llmseg_checkpoint", "") != args.checkpoint
                        or metadata.get("region_background_rgb", "") != args.region_background_rgb
                    ):
                        missing.append(f"stale:{metadata_path}")
                        continue

                    topk_records = []
                    for rank_info in metadata.get("ranks", []):
                        rank = int(rank_info["rank"])
                        mask_path = topk_mask_path(args, split_name, stem, rank)
                        region_path = topk_region_path(args, split_name, stem, rank)
                        ...
                        topk_records.append(
                            {
                                "rank": rank,
                                "pred_similarity": float(rank_info.get("pred_similarity", 0.0)),
                                "pred_iou": float(rank_info.get("pred_iou", 0.0)),
                                "llmseg_mask_path": str(mask_path),
                                "region_path": str(region_path),
                            }
                        )
                    if not topk_records:
                        missing.append(f"empty:{metadata_path}")
                        continue
                    ...
                    item["records"].append(
                        {
                            ...
                            "stem": stem,
                            "topk_records": sorted(topk_records, key=lambda row: row["rank"]),
                        }
                    )
    ...
    print(f"  Reused LLMSeg Top-K ranks per instruction: up to {args.topk_mask_k}")
...
def generate_region_inputs(llmseg, args: argparse.Namespace) -> Tuple[Dict[str, List[Dict]], Dict[str, List[Tuple[str, str]]]]:
    ...
    try:
        for split_name in ["easy", "hard"]:
            ...
            print("\n" + "=" * 60)
            print(f"  Stage 1: LLMSeg Top-{args.topk_mask_k} masks -> region RGB ({split_name})")
            print("=" * 60)
            seen_counts = defaultdict(int)
            ...
            for prepared_sample in tqdm(loader, desc=f"LLMSeg {split_name.capitalize()}"):
                ...
                try:
                    ...
                    for instruction_idx, instruction in enumerate(sample.get("instructions", [])[:3]):
                        pred_result = llmseg.predict_mask_llmseg(
                            model, tokenizer, prepared_sample, instruction, args
                        )
                        if not isinstance(pred_result, dict):
                            raise TypeError("LLMSeg Top-K inference must return a result dictionary")
                        top_masks = list(pred_result.get("top_masks", []))
                        top_scores = list(pred_result.get("top_scores", []))
                        top_pred_ious = list(pred_result.get("top_pred_ious", []))
                        if not top_masks:
                            top_masks = [pred_result["pred_mask"]]
                            top_scores = [0.0]
                            top_pred_ious = [0.0]
                        stem = output_stem(sample, obj_count, instruction_idx)
                        topk_records = []
                        metadata_ranks = []
                        for rank, top_mask in enumerate(top_masks[: args.topk_mask_k], start=1):
                            if top_mask.shape != (h, w):
                                top_mask = cv2.resize(
                                    top_mask.astype(np.float32),
                                    (w, h),
                                    interpolation=cv2.INTER_NEAREST,
                                )
                            vigor_mask = to_vigor_mask(top_mask)
                            pred_similarity = float(top_scores[rank - 1]) if rank <= len(top_scores) else 0.0
                            pred_iou = float(top_pred_ious[rank - 1]) if rank <= len(top_pred_ious) else 0.0

                            mask_path = topk_mask_path(args, split_name, stem, rank)
                            mask_path.parent.mkdir(parents=True, exist_ok=True)
                            if not cv2.imwrite(str(mask_path), vigor_mask):
                                raise OSError(f"Failed to save LLMSeg mask: {mask_path}")
                            ...
                            if not cv2.imwrite(
                                str(region_path), cv2.cvtColor(region_rgb, cv2.COLOR_RGB2BGR)
                            ):
                                raise OSError(f"Failed to save region RGB: {region_path}")
                            ...
                            topk_records.append(rank_record)
                        ...
                        save_topk_metadata(
                            topk_metadata_path(args, split_name, stem),
                            {
                                "version": 1,
                                "requested_topk_mask_k": int(args.topk_mask_k),
                                "llmseg_checkpoint": args.checkpoint,
                                "region_background_rgb": args.region_background_rgb,
...
def make_reranker_args(args: argparse.Namespace) -> SimpleNamespace:
    return SimpleNamespace(
...
def evaluate_with_vlpart(
    ...
    reranker_device = torch.device(
        args.reranker_device if torch.cuda.is_available() else "cpu"
    )
    reranker_args = make_reranker_args(args)
    ranker_model, ranker_mean, ranker_std, feature_indices, feature_names = (
        reranker.load_reranker_checkpoint(reranker_args, reranker_device)
    )
    print(f"  Reranker checkpoint: {args.reranker_checkpoint}")
    print(f"  Reranker features: {feature_names}")
    print(f"  VLPart candidates per rank: {args.vlpart_mask_selection}")
    results = {"easy": [], "hard": []}
    ...
    for split_name in ["easy", "hard"]:
        ...
        print("\n" + "=" * 60)
        print(f"  Stage 2: VLPart Top-K candidates + reranker ({split_name})")
        print("=" * 60)
        ...
        for item in tqdm(split_items, desc=f"VLPart {split_name.capitalize()}"):
            ...
            try:
                ...
                for record in item["records"]:
                    candidates = []
                    for topk_record in record["topk_records"]:
                        rank = int(topk_record["rank"])
                        region_path = topk_record["region_path"]
                        object_mask = cv2.imread(
                            topk_record["llmseg_mask_path"], cv2.IMREAD_GRAYSCALE
                        )
                        if object_mask is None:
                            raise FileNotFoundError(
                                f"Failed to read LLMSeg mask: {topk_record['llmseg_mask_path']}"
                            )
                        if object_mask.shape != (h, w):
                            object_mask = cv2.resize(
                                object_mask, (w, h), interpolation=cv2.INTER_NEAREST
                            )
                        object_fg = object_mask == 0

                        vlpart_candidates = pred_cache.get(region_path)
                        if vlpart_candidates is None:
                            region_bgr = cv2.imread(region_path, cv2.IMREAD_COLOR)
                            if region_bgr is None:
                                raise FileNotFoundError(f"Failed to read region RGB: {region_path}")
                            predictions = demo.predictor(region_bgr)
                            vlpart_candidates = reranker.instances_to_candidates(
                                predictions, demo.metadata, (h, w)
                            )
                            vlpart_candidates = reranker.apply_mask_selection_to_candidates(
                                vlpart_candidates, args.vlpart_mask_selection
                            )
                            ...
                            pred_cache.put(region_path, vlpart_candidates)

                        for candidate in vlpart_candidates:
                            pred_fg = candidate["pred_fg"]
                            contain = reranker.containment_score(pred_fg, object_fg)
                            features = [
                                float(topk_record["pred_similarity"]),
                                float(topk_record["pred_iou"]),
                                float(candidate["pred_score"]),
                                float(contain),
                            ]
                            candidates.append(
                                {
                                    ...
                                    "llmseg_mask_path": topk_record["llmseg_mask_path"],
                                    "pred_similarity": features[0],
                                    "pred_iou": features[1],
                                    "vlpart_score": features[2],
                                    "containment_score": features[3],
                                    "pred_class": int(candidate["pred_class"]),
                                    "pred_class_name": candidate["pred_class_name"],
                                    "vlpart_instance_index": int(candidate["vlpart_instance_index"]),
                                }
                            )

                    if not candidates:
                        raise RuntimeError(f"No candidates for instruction: {record['stem']}")
                    group = {
                        "features": np.asarray(
                    ...
                    best_idx, reranker_score = reranker.pick_candidate(
                        ranker_model,
                        ranker_mean,
                        ranker_std,
                        feature_indices,
                    ...
                    pred_fg = selected["pred_fg"]
                    pred_info = {
                        "pred_score": selected["vlpart_score"],
                        "pred_class": selected["pred_class"],
                        "pred_class_name": selected["pred_class_name"],
                    ...
                    selection_details.append(
                        {
                            "instruction_idx": int(record["instruction_idx"]),
                            "selected_topk_rank": int(selected["llmseg_topk_rank"]),
                            "vlpart_instance_index": int(selected["vlpart_instance_index"]),
                            "candidate_count": len(candidates),
                            "reranker_score": float(reranker_score),
                            "features": list(selected["features"]),
                            "llmseg_mask_path": selected["llmseg_mask_path"],
                        }
                    )
                    ...
                    if args.debug:
                        ...
                        print(
                            ...
                            f"rank={selected['llmseg_topk_rank']}, "
                            f"reranker={reranker_score:.4f}, candidates={len(candidates)}"
                        )
                ...
                results[split_name].append(
                    {
                        ...
                        "gt_mask_path": sample["gt_mask_paths"][0],
                        "gt_mask_source": sample.get("gt_mask_source", "gt_mask_path"),
                        "gt_object": sample["gt_object"],
                        "difficulty": sample["difficulty"],
                        "ic_ious": ic_ious,
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    if not Path(args.reranker_checkpoint).is_file():
        raise FileNotFoundError(f"Reranker checkpoint not found: {args.reranker_checkpoint}")
    resolve_run_output_dirs(args)

    print("=" * 72)
    print("  LLMSeg Top-K -> region RGB -> VLPart candidates -> reranker")
    print("=" * 72)
    print(f"Data dir: {args.data_dir}")
    print(f"LLMSeg checkpoint: {args.checkpoint}")
    print(f"VLPart weights: {args.vlpart_weights}")
    print(f"LLMSeg Top-K K: {args.topk_mask_k}")
    print(f"VLPart candidate mode: {args.vlpart_mask_selection}")
    print(f"Reranker checkpoint: {args.reranker_checkpoint}")
    ...
    reranker = load_reranker_module()
    results = evaluate_with_vlpart(
        llmseg, vlpart_eval, reranker, items, skipped_errors, args
    )
    ...
    print("\n" + "=" * 60)
    print("  LLMSeg Top-K + VLPart + reranker evaluation finished")
    print("=" * 60)
